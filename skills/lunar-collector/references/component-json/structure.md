...
| **[`.ai`](cat-ai.md)** | **AI coding assistant usage (code reviewers, instruction files, authorship)** |
| `.ai.code_reviewers[]` | Active AI code reviewers (`tool`, `check_name`, `detected`, `last_seen`) |
| `.ai.instructions.root` | Root instruction file info (`exists`, `filename`, `lines`, `bytes`, `sections[]`) |
| `.ai.instructions.all[]` | All instruction files (`path`, `dir`, `filename`, `lines`, `bytes`, `sections[]`, `is_symlink`) |
| `.ai.instructions.count` | Total instruction files found |
| `.ai.instructions.total_bytes` | Combined size of non-symlink files (context window budget) |
| `.ai.instructions.directories[]` | Per-directory view (`dir`, `files[]` with symlink status) |
| `.ai.plans_dir` | Plans directory info (`exists`, `path`, `file_count`) |
| `.ai.authorship` | AI authorship annotations (`provider`, `total_commits`, `annotated_commits`) |
| `.ai.authorship.git_ai` | Git AI notes data (`notes_ref_exists`, `commits_with_notes`) |
| `.ai.authorship.trailers` | Git trailer data (per-commit `sha`, `model`, `tokens`, `has_annotation`) |
